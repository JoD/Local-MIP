...
bool LocalMIP::SatTightMove(std::vector<uint8_t>& score_table, std::vector<size_t>& score_idx) {
  ...
  for (size_t time = 0; time < sampleSat; time++) {
    ...
    if (std::find(neighborConIdxs.begin(), neighborConIdxs.end(), conIdx) == neighborConIdxs.end() &&
