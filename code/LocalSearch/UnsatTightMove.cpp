...
bool LocalMIP::UnsatTightMove() {
  std::vector<uint8_t>& scoreTable = localVarUtil.scoreTable;
  ...
  if ((!isFoundFeasible && scoreSize > bmsUnsatInfeas) || (isFoundFeasible && scoreSize > bmsUnsatFeas)) {
    if (!isFoundFeasible)
  ...
  if (bestScore > 0) {
    ...
    for (auto idx : scoreIdxs) scoreTable[idx] = false;
    return true;
  } else {
