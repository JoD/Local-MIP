...
ModelVar::ModelVar(const string &_name, size_t _idx, bool _integrality)
  : name(_name), idx(_idx), upperBound(DefaultRealUpperBound),
    lowerBound(DefaultLowerBound), termNum(-1), type(VarType::Real) {
  if (_integrality) {
    type = VarType::Binary;
...
ModelVarUtil::ModelVarUtil()
  : isBin(true), varNum(-1), integerNum(0), binaryNum(0), fixedNum(0),
    realNum(0), objBias(0) {
}
...
ModelVar &ModelVarUtil::GetVar(const string &_name) {
